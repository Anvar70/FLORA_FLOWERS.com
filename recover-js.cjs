const content = `const BOOT=JSON.parse(document.getElementById('boot').textContent);
...
async function api(url,method='GET',data){
  ...
  if(!response.ok){const err=result.error||{};let code=err.code;if(Array.isArray(code))code=code[0];if(!code){const str=JSON.stringify(err);code=Object.keys(I18N).find(k=>str.includes('"'+k+'"'));}throw Error(t(code||(response.status===403?'not_allowed':response.status===429?'try_later':'invalid_form')));}
  return result;
}
let toastTimer;
...
async function render(preserve=false){
 ...
 }catch(e){$('#content').innerHTML='<div class="empty"><h3>'+t('error')+'</h3><p>'+esc(e.message)+'</p>'+button('retry','retry')+'</div>';}
}
...
document.addEventListener('click',async e=>{
 ...
 try{
  if(a==='menu'){const sidebar=$('#sidebar'),open=!sidebar.classList.contains('open');sidebar.classList.toggle('open',open);target.setAttribute('aria-expanded',String(open));if(open)$('a',sidebar).focus();}
 ...
 }catch(error){toast(error.message);}finally{if(a==='quick-add')target.disabled=false;}
});
document.addEventListener('change',async e=>{
 const el=e.target;try{
  ...
  if(el.name==='saved_address'){const a=window.addressCache.find(x=>x.id===Number(el.value));if(a){const f=$('#checkout-form');['recipient','phone','address'].forEach(k=>f.elements[k].value=a[k]);}}
 }catch(error){toast(error.message);}
});
document.addEventListener('keydown',e=>{
 if(e.key==='Escape'){closeModal();$('#sidebar')?.classList.remove('open');$('.mobile-menu')?.setAttribute('aria-expanded','false');}
 const modal=$('.modal');
...
document.addEventListener('submit',async e=>{
 ...
 const d=Object.fromEntries(new FormData(f));try{
  if(f.id==='auth-form'){const result=await api('/auth/'+authMode+'/','POST',{...d,role:authRole,language:lang});if(authMode==='reset')toast(t('reset_sent'));else location.href=result.redirect;}
   ...
   const o=await api('/api/checkout/','POST',{...d,key});sessionStorage.removeItem('flora-checkout-key');navigate(root()+'orders/'+o.id+'/?success=1');
  }
  ...
  else if(f.id==='email-form'){await api('/api/email/','POST',d);toast(t('saved'));await profilePage();}
...
async function poll(){if(!user||document.hidden||pollBusy)return;pollBusy=true;try{const d=await api('/api/dashboard/');applyCounts(d);if($('#chat-history'))await refreshMessages();}catch(e){if($('#chat-history'))toast(t('network_error'));}finally{pollBusy=false;}}
window.addEventListener('popstate',()=>{path=location.pathname;render();});
document.addEventListener('click',e=>{const side=$('#sidebar');if(side?.classList.contains('open')&&!side.contains(e.target)&&!e.target.closest('[data-action=menu]'))side.classList.remove('open');});
localStorage.setItem('flora-language',lang);render();setInterval(poll,12000);setTimeout(poll,1500);
`;
require('fs').writeFileSync(__dirname+'/static/app.js',content);